...
# --- Start Docker infrastructure (SWIM + NATS) ---
...
# NATS container
if docker ps --filter name=polaris-nats --filter status=running --format "{{.Names}}" | grep -q polaris-nats; then
    log_success "polaris-nats container already running"
elif docker ps -a --filter name=polaris-nats --format "{{.Names}}" | grep -q polaris-nats; then
    log_info "Starting existing polaris-nats container..."
    docker start polaris-nats
else
    log_info "Creating new polaris-nats container..."
...
if docker ps --filter name=swim --filter status=running --format "{{.Names}}" | grep -q swim; then
    log_success "swim container already running"
elif docker ps -a --filter name=swim --format "{{.Names}}" | grep -q swim; then
    log_info "Starting existing swim container..."
    docker start swim
else
    log_info "Creating new swim container..."
    docker run -d --name swim -p 4242:4242 -p 5901:5901 -p 6901:6901 gabrielmoreno/swim
fi

# Wait for SWIM container VNC desktop to initialise before launching simulation
log_info "Waiting for SWIM container to initialise (8s)..."
sleep 8

# Start the SWIM OMNeT++ simulation inside the container
log_info "Starting SWIM OMNeT++ simulation..."
...
log_info "Waiting for SWIM simulation to open port $SWIM_PORT..."
_swim_ready=false
for _i in $(seq 1 30); do
    if nc -z "$SWIM_HOST" "$SWIM_PORT" &>/dev/null; then
        _swim_ready=true
        break
    fi
    sleep 1
done
if [[ "$_swim_ready" != true ]]; then
    log_error "SWIM simulation failed to open port $SWIM_PORT — check container logs: docker logs swim"
    exit 1
fi
log_success "SWIM simulation is ready"
...
nats_command="echo 'NATS server running in Docker container polaris-nats' && "
nats_command+="docker logs -f polaris-nats"
